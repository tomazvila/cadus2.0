...
Fix wave done (2026-08-30 evening): all nine units gate green on their branches; merged through `m6/fix-a2` (fast-forward to `99d2901`; migration 0012 `content_prompt_digest`). Changes: the content_store key is `sha256(kp_id, kind, body)` and a duplicate or a refused re-author is reported as such; the hint/teach gates compare every rung against exemplar answers AND at least 8 rendered instances of every approved template (the exemption is removed; 307 knowledge points show an answer in an exemplar text, 62 had no other cover); the rework branch stops the countdown and the Retry re-enters through the phase gate; one history-based router, a 401 resets the view, a reset link works while signed in; Review's Approve/Reject bind to the loaded digest, the course picker is a `Modal`, the `/ops` bill pages through every row (`page` query on the R5 list route); `AUTHORING_OUTPUT_TOKENS` 4000 / `AUTHORING_REASONING_MAX_TOKENS` 2000 and the zero-call guard for `diagnosis`; the route table has a Rust-generated fixture (`web/src/api/routes.generated.json`, `npm run routes`); `deploy.sh` checks every started service twice 2 s apart, reloads Caddy through exec, and `--no-caddy` is removed; `check_ops` validates the Caddyfile in the caddy image, the mount, the `@ops` block, and the `.env.example` cookie pair; trap T1 `repair_latex_escapes` runs before every gate (a surviving control character refuses the body); `content_store.prompt_digest` is stored and `cadus-worker author --stale` lists the rows to re-author.

Gate on `main` at `99d2901` after the fix wave: `npm run check` PASS (CSP audit clean), Rust gate 1,800 tests, 0 failed (log gate-m6-2). Verification round (2026-08-31, `docs/reviews/M6-review-2.md`): 16 raised, 12 confirmed, nine distinct defects. Three re-open round-1 fixes: the give-away gate reads APPROVED templates only, so the single documented authoring pass gates every ladder against an empty answer set (blocker); `gate_teach` never reads `instance_answers` (the teach half of F15 was not implemented); the Caddy reload re-reads a stale inode after `git pull`. Six are new or test-quality: the quiz and placement Retry gates, the restartable quiz clock, a stale-row re-author that never clears its prompt digest, the CSP audit blind to inline scripts, the last-rung-only gate pin, the unpinned diagnosis timing literals. Fix wave 2 `wf_5f6ab89c-c38`: units FIX2-M6-A..E, then F merges all. A (`de635e9`), C (`1cb3853`), D (`73b076c`) and E (`07d4819`) are gate green; a process exit (the fifth of the build) interrupted B, whose work is preserved on `m6/fix2-b-wip` and merged by the resumed unit.

Unit FIX2-M6-G (branch `m6/fix2-g`) closes two residuals the wave-2 agents reported in their own runs:
...
- `Quiz.tsx` and `Diagnostic.tsx` still arm a Retry that re-enters `run()` past their phase gate (the `retryGate` option exists; two call sites not converted).
- `docker-compose.yml` does not forward the two authoring token variables to the worker; `check_ops` asserts the seven A4 variables only.
- `cadus_store::content::review_list` has no caller; the route's `review_page` duplicates the SELECT with an `OFFSET`.
- A `?reset=` token while signed in skips `me()` at boot, so the learner signs in again after the reset (documented; a session revoke on password change makes it the correct end state).
- The `document_digest` function lives in `cadus-worker`; rows written before this commit keep the old digest format (no data migration is possible; a NULL `prompt_digest` is never stale).
- The route fixture scans `.route("...")` string literals; a route built from a const or a macro escapes the scan.
- `types.ts` `ROUTES` docstring still describes the old chain.
- The rework re-solve clock counts up from 0:00 (display decision, unpinned by the spec).
- Two contended gate runs failed on benchmark B (long log) p95 while p50 held; the committed trees passed on re-run. One full gate at a time on this box.
...
**CLOSED 2026-09-01 by unit M7-P (the placement API).** The three routes are mounted and the shipped router has 38. What landed:

- `crates/core/src/diagnostic.rs`, the port of `cadus/diagnostic.py`: the probe set (a greedy set cover over the two demands of every in-scope topic, on bit masks like `selector::compress`), the accuracy-time answer weight, the propagation (a correct answer credits every ancestor, an incorrect one debits every descendant, a leaf passes `sibling_credit` of the signed weight to its sibling leaves), the next-probe choice, and the placement. It seeds no FIRe state: `Projector::on_diagnostic_placed` alone reads a balance, off the event.
- `crates/store/src/state.rs`: `load_diag_state`, `save_diag_state`, `clear_diag_state`. The `diag_states` table has been in migration 0004 since M3 and nothing read it.
- `crates/web/src/diag.rs`: the three handlers. ONE transaction per call, because the verdict is a local `check` and not a model round trip (1.0 splits `answer` into three phases to drop the tenant lock across its LLM call). The served probe hangs on the `"diag"` sentinel task id of the D-S6 document, so a superseded id answers `404 unknown_problem`.
- The deployment rule this service adds to 1.0: `start` drops from the probe set every topic whose `answer_kind` has no deterministic checker, so the diagnostic never asks a question this service cannot mark (R4, L6). 1.0 sent such a probe to its grading engine.
- The SPA: the three calls are `ApiClient` members now, on the live client and on the demo client, and `SPEC_ROUTES_ABSENT` is down to `POST /api/task/{task_id}/abort` (S8's exit path). `api/diag.ts` keeps the port, because the placement screen still takes its transport as a prop.
- Tests: 19 in `crates/core/tests/diagnostic.rs` (the cover holds the extremes, a smaller radius never needs fewer probes, the weight floor, the three propagation rules, the tie to the lowest id, the question cap, the four placement bands, the document round trip) and 10 in `crates/web/tests/diag_route.rs` (the whole loop with both events appended, no expected answer and no solution in either reply, the blank skip, the first-run enroll, a second start superseding the first, and the four refusals: `404 unknown_course`, `409 no_diagnostic` on answer and on finish, `422 invalid_request`, `404 unknown_problem`).

Still open from the same check: `POST /api/task/{task_id}/abort` (S8), which `SPEC_ROUTES_ABSENT` records.

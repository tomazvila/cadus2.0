...
| **T21** | **An optimized build rewrites `powf` with a LITERAL base.** LLVM turns `0.5_f64.powf(x)` into `exp2(-x)`, and glibc `exp2` differs from glibc `pow` by one unit in the last place on about 0.1 percent of real fold exponents. CPython `0.5 ** x` calls `pow`. Measured: 3 of the 913 `memory_at` exponents of `stream_9.jsonl` diverge, and 8 parity tests fail in `--release` while every one of them passes in the debug profile the plain `cargo test` uses. The Docker image builds with `--release`, so the divergent path is the one that ships. | `fire.py:179` (`memory_at`); `xp.py:113` (the blow-off escalation) | Hide the base behind `std::hint::black_box` at every `powf` with a literal or constant base, so the compiler emits the real `pow` call: `crates/core/src/fire.rs` `pow_half`, `crates/core/src/xp.rs` `quality_multiplier`. Write NO literal base at a new `powf` site. `scripts/gate.sh` folds the 20 committed streams a second time with `cargo test --release -p cadus-core --test parity_events --test projector`; the digests must hold in BOTH profiles. |
| **T22** | **CPython raises where Rust returns a number, in two places the fold reaches.** (a) `0.5 ** exponent` raises `OverflowError: (34, 'Numerical result out of range')` for a finite exponent below about -1074, while `f64::powf` returns `inf`; the 1.0 fold then builds NO model. (b) Python `int()` is unbounded, so `int(round(x))` of an XP total outside the `i64` range is an exact big integer, while a Rust cast saturates. Both are reachable from one schema-valid event: a `t0` later than the read instant by more than about 1024 intervals for (a), and an `xp` of `-1e308` for (b). | (a) `fire.py:179`; (b) `projector.py:623`, `:632`, `:633` | (a) The fold stops: `apply_attempt_checked` reports the topic and `project` returns `ProjectorError::NonFinite { topic, event_index }`, which is the 1.0 outcome — no model. (b) A DOCUMENTED DIVERGENCE: `round_half_even_i64` returns `ProjectorError::OutOfRange` where 1.0 keeps the big integer, because the learner model holds an `i64`. The range is `-2**63` to `2**63` exclusive, so the largest value it holds is `9223372036854774784.0`. Only a call site with an input bounded by construction uses `round_half_even_i64_saturating`. |

...
|---|---|---|---|---|---|---|
| (a) | The grade route accepts `numeric` and `expression` only | — (f4-outcome) | — (f4) | `docs/reports/foundations-answer-inventory.md` §0: 233 `multi-step` answers decide today and the kind gate refuses all 233 | f1 measures the loss. The gate stays until f4 replaces it with the contract gate. | `open` |
...
| (c) | The verdict converts `Undecidable` into incorrect | — (f4-outcome) | — (f4) | `docs/reports/foundations-answer-inventory.md` §2: 370 of 1,695 authored answers are undecidable today | f1 measures the authored side only. The learner side needs the f4 outcome split. | `open` |
| (d) | `sqrt(2)` against `2^(1/2)` gives undecidable | — (f2-grammar) | — (f2) | `docs/reports/foundations-answer-inventory.md` §5.1 `rational_exponent`: 3 authored answers, example `x^(1/2)` in `radical-exponent-conversion` | The learner-side pair is not in the inventory: f1 reads authored answers only. | `open` |
...
| (f) | Exemplar parsing refuses `9 R2` | — (f2-grammar) | `crates/core/tests/answer_inventory.rs` | `docs/reports/foundations-answer-inventory.md` §2 and §5.1: 12 `quotient_remainder` answers, 0 decided, both spellings | No `q R r` production yet. 12 answers stay outside the pool. | `open` |
| (g) | Work-quality tiers derive from final-answer correctness only | — (f4-outcome, D-F4) | — (f4) | — | Out of f1 scope. The inventory reads answers, not reasoning. | `open` |
...
| (j) | The SPA falls back to practice when teach fails | — (f6-readiness-core) | — (f6) | — | Out of f1 scope. | `open` |
| (k) | `lesson.kp_pass` is not read by the gate | — (f12-pass-rule) | — (f12) | — | Out of f1 scope. | `open` |
| (l) | `xp::is_mastered` includes Learning, Placed, and Floor | — (f14-mastery-split) | — (f14) | — | Out of f1 scope. | `open` |

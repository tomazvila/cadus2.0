...
pub fn write(value: &Ast) -> Result<String, EvalError> {
    if holds_an_evaluation_only_name(value) {
...
/// Write a tree that holds no evaluation-only name.
///
/// [`super::evaluate`] erases every such name or refuses the tree, so the
/// answer path writes its value through this function and never fails.
pub(super) fn write_tree(value: &Ast) -> String {
    let mut out = String::new();
    write_at(value, Prec::Lowest, &mut out);
    out
}

/// Whether the tree holds a function name the writer may not put back (V2).
fn holds_an_evaluation_only_name(node: &Ast) -> bool {
    match node {
        Ast::Func(name, args) => {
            !is_writable_function(name) || args.iter().any(holds_an_evaluation_only_name)
        }
        ...
        | Ast::Tuple(items)
        | Ast::Set(items)
        | Ast::List(items) => items.iter().any(holds_an_evaluation_only_name),
        Ast::Div(left, right)
        | Ast::Interval {
            lo: left,
            hi: right,
...
fn write_at(node: &Ast, need: Prec, out: &mut String) {
...
fn write_bare(node: &Ast, out: &mut String) {
    match node {
...
fn write_fraction(numerator: &BigInt, denominator: &BigInt, out: &mut String) {
...
fn write_mixed(whole: &BigInt, numerator: &BigInt, denominator: &BigInt, out: &mut String) {
...
fn write_negation(inner: &Ast, out: &mut String) {
    out.push('-');
    write_at(inner, Prec::Power, out);
}

/// Write `sqrt(…)`.
fn write_root(inner: &Ast, out: &mut String) {
    out.push_str("sqrt(");
...
fn write_power(base: &Ast, exponent: i64, out: &mut String) {
...
fn write_wrapped(items: &[Ast], delimiters: (char, char), out: &mut String) {
...
fn write_interval(lo: &Ast, hi: &Ast, lo_closed: bool, hi_closed: bool, out: &mut String) {
...
fn write_inequality(var: &str, op: IneqOp, bound: &Ast, out: &mut String) {
    out.push_str(var);
    out.push(' ');
...
fn write_assignment(var: &str, value: &Ast, out: &mut String) {
    out.push_str(var);
    out.push_str(" = ");
...
fn write_chain(lo: &Ast, lo_closed: bool, var: &str, hi_closed: bool, hi: &Ast, out: &mut String) {
    write_at(lo, Prec::Lowest, out);
...
fn write_joined(items: &[Ast], separator: &str, need: Prec, out: &mut String) {

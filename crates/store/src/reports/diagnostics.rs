...
pub async fn start(
    ...
    let seq: i64 = sqlx::query_scalar("SELECT coalesce(max(seq),0) FROM events WHERE user_id=$1")
        ...
        .await?;
    sqlx::query(
        "UPDATE problem_report_diagnostics SET end_seq=$2 WHERE user_id=$1 AND end_seq IS NULL",
    )
    .bind(user)
    .bind(seq)
    .execute(&mut **tx)
...
pub async fn save(
    ...
    sqlx::query(
        "UPDATE problem_report_diagnostics SET state=$2 WHERE user_id=$1 AND end_seq IS NULL",
    )
    .bind(user)
    .bind(state)
    .execute(&mut **tx)
...
pub(super) async fn complete_regrade(
    ...
    let seq = packet["event_seq"]
        .as_i64()
        .ok_or_else(|| invalid("missing diagnostic sequence"))?;
    let run = for_answer(tx, user, seq)
        .await?
        .ok_or_else(|| invalid("diagnostic run predates repair tracking"))?;
    let policy = &packet["task_policy"];
    if run["run_id"] != policy["diagnostic_run_id"]
        || run["curriculum_digest"] != policy["diagnostic_curriculum_digest"]
    {
    ...
    let mut state: DiagState = serde_json::from_value(run["state"].clone())
    ...
    fixed.weight = serde_json::from_value(policy["diagnostic_weight"].clone())
        .map_err(|error| invalid(&error.to_string()))?;
    super::task_outcomes::replace_event(correction, seq, Event::DiagnosticAnswer(fixed))?;
    let config: Config = serde_json::from_value(run["config"].clone())
        .map_err(|error| invalid(&error.to_string()))?;
    let calculated = placement(&state, &config);
    if run["completed"] == true {
        let close_seq = run["end_seq"]
            .as_i64()
            .ok_or_else(|| invalid("missing diagnostic close"))?;
        let Some(Event::DiagnosticPlaced(original)) = rows
            .iter()
            .find(|row| row.seq == close_seq)
            .map(|row| &row.event)
        else {
    ...
    sqlx::query("UPDATE problem_report_diagnostics SET state=$3 WHERE user_id=$1 AND run_id=$2")
        ...
        .await?;
    Ok(
        json!({"status":"recalculated","placement_completed":run["completed"],"balances":calculated.event_balances(),"xp":0}),
    )
}
...
fn apply_delta(state: &mut DiagState, delta: &Value) -> Result<(), StoreError> {
    let delta = delta
        .as_object()
        .ok_or_else(|| invalid("missing diagnostic delta"))?;
    for (topic, change) in delta {
        let amount = change
            .as_f64()
            .filter(|value| value.is_finite())
            .ok_or_else(|| invalid("invalid diagnostic delta"))?;
        let balance = state
            .balances
            .get_mut(topic)
            .ok_or_else(|| invalid("diagnostic delta exceeds frozen universe"))?;
        *balance += amount;
        if !balance.is_finite() {
            return Err(invalid("diagnostic balance overflow"));
        }
    }
    Ok(())
}
...
fn invalid(message: &str) -> StoreError {
    StoreError::Document(message.into())
}
...
mod tests {
    ...
    fn placement_repair_retains_full_universe_and_cumulative_corrections() {
        let mut state: DiagState =
            serde_json::from_value(json!({"balances":{"a":-1.0,"b":0.0},"answered":["a"]}))
                .unwrap();
        apply_delta(&mut state, &json!({"a":2.0,"b":1.0})).unwrap();
        let first = placement(&state, &Config::default());
        assert_eq!(first.placed["a"], 1.0);
        assert_eq!(first.placed["b"], 1.0);

...
pub(super) async fn complete_submission(
    tx: &mut Transaction<'_, Postgres>, user: Uuid, packet: &Value, report: Uuid,
    ...
    let (task_id, topic, session, attempts) = match &original {
        Event::Attempt(attempt) => {
            if attempt.correct { return Ok((None, json!({"status":"already_correct"}))); }
            (attempt.task_id.clone(), attempt.topic.clone(), attempt.session.clone(), vec![RegradedAttempt {
                attempt_id: attempt.attempt_id.clone(), outcome: Some(AttemptOutcome::Correct),
                work_quality: WorkQuality::NearlyPerfect, error_tags: Vec::new(),
                grader_note: Some(format!("Qwen report {report}; checked mathematical evidence")),
            }])
        }
        Event::IntegratedAttempt(attempt) => (attempt.task_id.clone(),
            Slug::new(&attempt.topic).map_err(|error| StoreError::Document(error.to_string()))?,
            attempt.session.clone(), Vec::new()),
        Event::DiagnosticAnswer(attempt) => (packet["report_identity"]["task_id"].as_str()
            .ok_or_else(|| StoreError::Document("diagnostic report has no task identity".into()))?.to_owned(),
            attempt.topic.clone(), attempt.session.clone(), Vec::new()),
        _ => return Err(StoreError::Document("unsupported report event".into())),
    };
    let micros = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH)
        .map_err(|error| StoreError::Document(error.to_string()))?.as_micros();
    let mut correction = Event::Regraded(Regraded {
        ts: Timestamp::from_micros(i64::try_from(micros).map_err(|error| StoreError::Document(error.to_string()))?),
        session, v: SchemaVersion::current(), task_id, topic, attempts, quality_tier: None, xp: None,
    ...
    let summary = match original {
        ...
        Event::IntegratedAttempt(_) => complete_integrated_regrade(tx, user, packet, &mut correction).await?,
        Event::DiagnosticAnswer(_) => super::diagnostics::complete_regrade(tx, user, packet, &mut correction).await?,
...
pub(super) async fn complete_regrade(
    ...
    let Event::Regraded(correction) = event else {
        return Err(StoreError::Document("task repair requires a regrade".into()));
    };
    ...
    let incomplete = replacements.iter().any(|replacement| replacement.event.is_none()
        && rows.iter().any(|row| row.seq == replacement.seq && matches!(row.event, Event::LessonResult(_))));
    if incomplete {
    ...
    let repair = Repair { reason: correction.reason.clone(), replacements };
    correction.reason = format!("{PREFIX}{}", serde_json::to_string(&repair)
        .map_err(|error| StoreError::Document(error.to_string()))?);
    Ok(summary)
}

async fn repair_quiz_scratch(tx: &mut Transaction<'_, Postgres>, user: Uuid, packet: &Value) -> Result<(), StoreError> {
    if packet["attempt"]["task_type"] != "quiz" { return Ok(()); }
    let Some(mut scratch) = load_web_state(tx,user).await? else { return Ok(()) };
    if scratch["session"] != packet["attempt"]["session"] { return Ok(()); }
    let Some(task) = packet["attempt"]["task_id"].as_str() else { return Ok(()) };
    let Some(problem) = packet["report_identity"]["problem_id"].as_str()
        .or_else(|| packet["problem_id"].as_str()) else { return Ok(()) };
    let Some(answers) = scratch["quizzes"][task]["answers"].as_array_mut() else { return Ok(()) };
    for answer in answers.iter_mut().filter(|answer| answer["problem_id"] == problem) {
    ...
    if all_correct {
        if let Some(pending) = scratch["feedback_practice"].as_object_mut() { pending.remove(task); }
        if let Some(progress) = scratch["tasks"].get_mut(task) {
            progress["done"] = json!(progress["total"].as_i64().unwrap_or(0) > 0
                && progress["answered"].as_i64().unwrap_or(0) >= progress["total"].as_i64().unwrap_or(0));
        }
    }
...
async fn reopen_lesson(
    tx: &mut Transaction<'_, Postgres>, user: Uuid, rows: &[EventRow], policy: &Policy,
    correction: &Regraded,
) -> Result<(), StoreError> {
    let Some(mut scratch) = load_web_state(tx, user).await? else { return Ok(()) };
    ...
    let attempts: Vec<_> = effective.iter().filter_map(|event| match event {
        Event::Attempt(attempt) if attempt.task_id == correction.task_id && !attempt.assisted
            && !attempt.feedback_practice && !attempt.outcome.is_ungraded() => Some(attempt),
        _ => None,
    }).collect();
    let next = policy.knowledge_points.iter().find(|kp| {
        let sequence: Vec<_> = attempts.iter().filter(|a| a.kp.as_ref().map(|id| id.as_str())
            .or_else(|| policy.knowledge_points.first().map(String::as_str)) == Some(kp.as_str()))
            .map(|a| a.correct).collect();
...
pub(super) fn replace_event(correction: &mut Event, seq: i64, replacement: Event) -> Result<(), StoreError> {
    let Event::Regraded(correction) = correction else {
        return Err(StoreError::Document("event repair requires a regrade".into()));
    };
    let mut repair = if let Some(encoded) = correction.reason.strip_prefix(PREFIX) {
        serde_json::from_str::<Repair>(encoded).map_err(|error| StoreError::Document(error.to_string()))?
    } else { Repair { reason: correction.reason.clone(), replacements: Vec::new() } };
    repair.replacements.push(Replacement { seq, event: Some(replacement) });
    correction.reason = format!("{PREFIX}{}", serde_json::to_string(&repair)
        .map_err(|error| StoreError::Document(error.to_string()))?);
    Ok(())
}
...
pub(super) async fn complete_integrated_regrade(
    tx: &mut Transaction<'_, Postgres>, user: Uuid, packet: &Value, correction: &mut Event,
) -> Result<Value, StoreError> {
    let seq = packet["event_seq"].as_i64()
        .ok_or_else(|| StoreError::Document("integrated report has no event sequence".into()))?;
    let field = packet["report_identity"]["field"].as_str()
    ...
    let Some(Event::IntegratedAttempt(attempt)) = rows.iter().find(|row| row.seq == seq).map(|row| &row.event) else {
        return Err(StoreError::Document("integrated report target is unavailable".into()));
    };
    let Event::Regraded(envelope) = correction else {
        return Err(StoreError::Document("integrated report requires a regrade".into()));
    };
    if attempt.task_id != envelope.task_id {
        return Err(StoreError::Document("integrated report task changed".into()));
    }
...
fn correct_integrated_field(attempt: &mut cadus_core::event::IntegratedAttempt, field_id: &str) -> Result<(), StoreError> {
    use cadus_core::event::AttemptOutcome;
    let field = if field_id == "final" { Some(&mut attempt.final_field) }
        else { attempt.steps.iter_mut().find(|field| field.id == field_id) }
    ...
    for field in attempt.steps.iter().chain(std::iter::once(&attempt.final_field)) {
        if field.outcome == AttemptOutcome::Correct {
            for skill in &field.skills {
                if !attempt.skills_credited.contains(skill) { attempt.skills_credited.push(skill.clone()); }
            }
        }
    }
    if let Some(grade) = &mut attempt.grade {
        for field in grade.steps.iter_mut().chain(std::iter::once(&mut grade.final_grade)) {
            if field.id == field_id {
                field.correct = true; field.ungraded = false; field.answered = true; field.notation = false;
            }
        }
        grade.correct_steps = grade.steps.iter().filter(|field| field.correct).count();
        grade.solved = attempt.solved;
        grade.ungraded = grade.steps.iter().chain(std::iter::once(&grade.final_grade)).any(|field| field.ungraded);
...
fn apply_overlay(rows: &mut Vec<EventRow>, corrections: &[Event]) -> Result<(), StoreError> {
    ...
    for event in corrections {
        let Event::Regraded(correction) = event else { continue };
        for attempt in &correction.attempts {
            attempts.insert(attempt.attempt_id.as_str(), (correction.task_id.as_str(), attempt));
        }
    ...
    rows.retain_mut(|row| {
        if let Some(replacement) = replacements.get(&row.seq) {
            let Some(event) = replacement else { return false };
...
fn recalculate(rows: &[EventRow], policy: &Policy, correction: &Regraded) -> Vec<Replacement> {
    ...
    events.push(Event::Regraded(correction.clone()));
    let effective = apply_regrades(&events);
    let attempts: BTreeMap<_, _> = effective.iter().filter_map(|event| match event {
    ...
    for row in rows {
        ...
        let replacement = match &row.event {
            ...
            Event::ReviewResult(result) if result.task_id.as_deref()
                .filter(|id| !id.is_empty()).map_or(bound, |id| id == correction.task_id)
                && result.topic == correction.topic => {
                let originals: Vec<_> = seen.iter().copied().filter(|a| !a.feedback_practice).collect();
                ...
                fixed.quality_tier = originals.last().map_or(result.quality_tier, |a| a.work_quality);
                fixed.xp = if fixed.inconclusive { 0.0 } else {
                    task_xp(TaskType::Review, fixed.quality_tier, &policy.config, 1, 0, false)
                };
            ...
            Event::RemediationTriggered(remediation) if repair_remediation
                && remediation.source_topic == correction.topic
                && (remediation.kind == "lesson_fail" || remediation.kind == "repeat_fail"
                    || remediation.kind.starts_with("review_confirmation:")) => {
                // Review remediation still required by the recomputed result is retained.
                let still_required = replacements.iter().rev().find_map(|r: &Replacement| match &r.event {
                    Some(Event::ReviewResult(result)) => Some(result.confirmation_skills.iter().any(|skill| {
                        skill.split_once('/').is_some_and(|(_, kp)| remediation.kind == format!("review_confirmation:{kp}"))
                    })),
                    _ => None,
                }).unwrap_or(false);
                if still_required { None } else { Some(None) }
            }
            _ => { repair_remediation = false; None }
        };
        if let Some(event) = replacement {
            replacements.push(Replacement { seq: row.seq, event });
        }
    }
    replacements
}
...
fn lesson_result(
    ...
    let independent: Vec<_> = attempts.iter().copied()
        .filter(|a| !a.assisted && !a.feedback_practice && !a.outcome.is_ungraded()).collect();
    ...
    for kp in &policy.knowledge_points {
        let sequence: Vec<bool> = independent.iter().filter(|a| {
            a.kp.as_ref().map(|id| id.as_str()).or_else(|| policy.knowledge_points.first().map(String::as_str)) == Some(kp.as_str())
        }).map(|a| a.correct).collect();
        let passed = kp_passed(&sequence, cfg.lesson.pass_rule());
        all_passed &= passed;
        if kp_failed(&sequence, cfg) && failed.is_none() { failed = Some(kp); }
    }
    ...
    if !all_passed && failed.is_none() { return None; }
    ...
    let count = if all_passed { i64::try_from(policy.knowledge_points.len()).unwrap_or(i64::MAX) } else { 1 };
    fixed.xp = task_xp(TaskType::Lesson, fixed.quality_tier, cfg, count, 0,
        !all_passed && is_rushing(last.correct, last.secs.get(), policy.expected_time_secs));
    Some(fixed)
}

#[expect(clippy::cast_precision_loss, reason = "a quiz contains a bounded question batch")]
fn quiz_result(original: &cadus_core::event::QuizResult, attempts: &[&Attempt], cfg: &Config) -> Event {
    let originals: Vec<_> = attempts.iter().copied().filter(|a| !a.feedback_practice).collect();
    let decided: Vec<_> = originals.iter().copied().filter(|a| !a.outcome.is_ungraded()).collect();
    let mut fixed = original.clone();
    ...
    fixed.per_topic = decided.iter().map(|a| QuizTopicResult {
        topic: a.topic.clone(), correct: a.correct, secs: a.secs,
    }).collect();
...
mod tests {
    ...
    fn attempt(id: &str, correct: bool, kind: &str) -> Event {
        event(json!({"type":"attempt","ts":"2026-01-01T12:00:00Z","session":"s",
    ...
    fn correction(id: &str) -> Regraded {
        let Event::Regraded(result) = event(json!({"type":"regraded","ts":"2026-01-02T12:00:00Z",
            ...
            "attempts":[{"attempt_id":id,"outcome":"correct","work_quality":"nearly_perfect"}]})) else { unreachable!() };
        result
    }
    fn policy() -> Policy {
        Policy { config: Config::default(), knowledge_points: vec!["k".into()], expected_time_secs: 60 }
    }
    fn rows(events: Vec<Event>) -> Vec<EventRow> {
        events.into_iter().enumerate().map(|(idx, event)| EventRow { seq: i64::try_from(idx).unwrap()+1, event }).collect()
    }
    fn lesson() -> Event {
        event(json!({"type":"lesson_result","ts":"2026-01-01T12:01:00Z","session":"s",
            "topic":"q","passed":false,"failed_at_kp":"k","xp":0,"quality_tier":"poor"}))
    }
    #[test]
    fn corrected_lesson_reprices_original_close_and_session_xp() {
        let mut rows = rows(vec![event(json!({"type":"session_start","ts":"2026-01-01T11:00:00Z","session":"s"})),
            attempt("a1", true, "lesson"), attempt("a2", false, "lesson"), lesson()]);
        let original = rows.last().unwrap().event.clone();
        let mut correction = correction("a2");
        let replacements = recalculate(&rows, &policy(), &correction);
        let Some(Event::LessonResult(result)) = &replacements[0].event else { panic!() };
        assert!(result.passed);
        assert_eq!(result.xp, task_xp(TaskType::Lesson, WorkQuality::NearlyPerfect, &policy().config, 1, 0, false));
        assert_eq!(rows.last().unwrap().event, original);
        correction.reason = format!("{PREFIX}{}", serde_json::to_string(&Repair { reason:"report".into(), replacements }).unwrap());
        ...
        assert_eq!(once, rows.iter().map(|r| r.event.clone()).collect::<Vec<_>>());
    }
    #[test]
    fn missing_knowledge_point_evidence_cannot_award_a_lesson_pass() {
        let rows = rows(vec![attempt("a1", true, "lesson"), attempt("a2", false, "lesson"), lesson()]);
        let mut policy = policy(); policy.knowledge_points.push("unanswered".into());
    ...
    fn review_recomputes_inconclusive_score_and_xp() {
        let rows = rows(vec![attempt("a1", true, "review"), attempt("a2", false, "review"),
            event(json!({"type":"review_result","ts":"2026-01-01T12:01:00Z","topic":"q",
        ...
        let Some(Event::ReviewResult(result)) = &repaired[0].event else { panic!() };
        assert!(result.passed); assert!(!result.inconclusive); assert_eq!(result.weighted_score, 1.0); assert!(result.xp > 0.0);
    }
    #[test]
    fn later_report_uses_earlier_effective_attempts_and_latest_price() {
        let mut rows = rows(vec![attempt("a1", false, "quiz"), attempt("a2", false, "quiz"),
            event(json!({"type":"quiz_result","ts":"2026-01-01T12:01:00Z","quiz_id":"t","score":0,"xp":0}))]);
        rows.push(EventRow { seq:4, event: Event::Regraded(correction("a1")) });
        let repaired = recalculate(&rows, &policy(), &correction("a2"));
        let Some(Event::QuizResult(result)) = &repaired[0].event else { panic!() };
        assert_eq!(result.score, 1.0); assert!(result.xp > 0.0); assert_eq!(result.per_topic.len(), 2);
    }
    #[test]
    fn drill_correction_preserves_its_non_scored_closure() {
        let rows = rows(vec![attempt("a1", false, "drill"), event(json!({"type":"drill_result",
    ...
    fn another_task_on_the_same_topic_keeps_its_close() {
        ...
        if let Event::Attempt(attempt) = &mut other { attempt.task_id = "other-task".into(); }
    ...
    fn integrated_field_repair_rebuilds_aggregate_without_crediting_other_misses() {
        let field = |id: &str, skill: &str| json!({"id":id,"answer":"2","contract":cadus_core::answer::AnswerContract::Exact,
            "outcome":"incorrect","assisted":false,"skills":[skill]});
        let grade = |id: &str, skill: &str| json!({"id":id,"answered":true,"correct":false,
        ...
        assert!(!attempt.solved); assert_eq!(attempt.skills_credited, ["q/k1"]);
        assert_eq!(attempt.grade.as_ref().unwrap().correct_steps, 1);
        correct_integrated_field(&mut attempt,"final").unwrap();
        assert!(attempt.solved); assert!(attempt.grade.as_ref().unwrap().solved);
    ...
    fn latest_repair_overlays_original_sequence_and_keeps_replay_invalidator() {
        ...
        if let Event::LessonResult(result) = &mut replacement { result.xp = 2.0; }
        replace_event(&mut first, 1, replacement.clone()).unwrap();
        let mut second = Event::Regraded(correction("a2"));
        if let Event::LessonResult(result) = &mut replacement { result.xp = 5.0; }
        ...
        assert_eq!(rows.iter().map(|row| row.seq).collect::<Vec<_>>(), [1,2,3]);
        let Event::LessonResult(result) = &rows[0].event else { panic!() };
    ...
    async fn integrated_repair_overlays_exact_submission_and_preserves_raw_fields() {
        crate::test_support::TestDb::with(|db| async move {
            ...
            let original = event(json!({"type":"integrated_attempt","ts":"2026-01-01T12:00:00Z",
            ...
            crate::state::append_event(&mut tx,user,&original,Some("integrated-a1")).await.unwrap();
            let mut correction = Event::Regraded(correction("unrelated"));
            if let Event::Regraded(body) = &mut correction { body.attempts.clear(); }
            let packet = json!({"event_seq":1,"report_identity":{"field":"final"}});
            complete_integrated_regrade(&mut tx,user,&packet,&mut correction).await.unwrap();
            ...
            let Event::IntegratedAttempt(attempt) = &effective[0].event else { panic!() };
            assert!(attempt.solved);
            assert_eq!(attempt.skills_credited,["q/k"]);
            assert_eq!(attempt.final_field.outcome,cadus_core::event::AttemptOutcome::Correct);
            let raw = crate::state::load_raw_events_after(&mut tx,user,0).await.unwrap();
            assert_eq!(raw[0].event,original);
            assert_eq!(raw.len(),2);
            let repeated = crate::integrated::attempt(&mut tx,user,"integrated-a1").await.unwrap().unwrap();

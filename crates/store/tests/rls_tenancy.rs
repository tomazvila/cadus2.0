...
/// D9: a second migration run applies nothing and leaves the 23 migration rows.
#[tokio::test]
...
/// Report queue reads and input inserts keep the same tenant boundary as events.
#[tokio::test]
async fn report_queue_isolates_tenants_and_refuses_cross_tenant_input() {
    TestDb::with(|db| async move {
        let a = db.seed_user("report-tenant-a@example.test").await;
        let b = db.seed_user("report-tenant-b@example.test").await;
        let insert = "INSERT INTO problem_reports
            (user_id,request_id,task_id,problem_id,attempt_id,source_hash,input)

...
async fn signed_in_problem_report_browser() {
    TestDb::with(|db| async move {
        ...
        println!(
            "Browser fixture ready on 127.0.0.1:{port}; manifest=/tmp/cadus-report-browser.json"
        );
        axum::serve(listener, app)
            .with_graceful_shutdown(async {
                for _ in 0..3600 {
                    if std::path::Path::new("/tmp/cadus-report-browser.stop").exists() {
                        return;
                    }

...
use cadus_store::{Db, DEFAULT_CLIENT_TIMEOUT_MS};
...
async fn create(app: &Router, user: Uuid, task: &str, mut body: Value) -> (StatusCode, Value) {
    ...
    let (status, raw) = call(app, Method::POST, &format!("/api/task/{task}/report"), Some(user), Some(body)).await;
    (status, parse(&raw))
}
async fn poll(app: &Router, user: Uuid, id: &Value) -> Value {
    let (status, raw) = call(app, Method::GET, &format!("/api/reports/{}",id.as_str().unwrap()),Some(user),None).await;
    assert_eq!(status,StatusCode::OK,"{raw}");
    parse(&raw)
}
async fn terminal(db: &TestDb, id: &Value) {
    sqlx::query("UPDATE problem_reports SET status='completed',stage='completed',result=$2 WHERE id=$1")
        .bind(Uuid::parse_str(id.as_str().unwrap()).unwrap())
        .bind(json!({"resolution":"confirmed_issue","solution":SECRET,"corrected_answer":"2",
...
async fn input(db: &TestDb, id: &Value) -> Value {
    sqlx::query_scalar("SELECT input FROM problem_reports WHERE id=$1")
        .bind(Uuid::parse_str(id.as_str().unwrap()).unwrap()).fetch_one(&db.admin).await.unwrap()
}
async fn quiz_attempt(db: &TestDb, user: Uuid, task: &str) {
    ...
    let mut event = attempt_payload(task,"quiz-report-attempt","kp1",(PROBLEM_TEXT,EXPECTED_ANSWER),&a_miss());
...
async fn quiz_closed(db: &TestDb, user: Uuid, task: &str, seq: i64) {
    let event = cadus_core::event::Event::from_json(&json!({"type":"quiz_result","ts":"2026-01-01T00:01:00Z",
        "session":SESSION,"quiz_id":task,"score":0.5,"xp":7.5,"per_topic":[]}).to_string()).unwrap();
...
async fn quiz_report_details_require_the_matching_native_quiz_close() {
    TestDb::with(|db|async move{
        ...
        let (status,queued)=create(&app,user,QUIZ,json!({"problem_id":PROBLEM_ID,"attempt_id":"quiz-report-attempt"})).await;
        assert_eq!(status,StatusCode::OK,"{queued}");
        terminal(&db,&queued["report_id"]).await;
        assert!(!poll(&app,user,&queued["report_id"]).await.to_string().contains(SECRET));
        quiz_closed(&db,user,"other-assessment",6).await;
        assert!(!poll(&app,user,&queued["report_id"]).await.to_string().contains(SECRET));
        quiz_closed(&db,user,QUIZ,7).await;
        assert_eq!(poll(&app,user,&queued["report_id"]).await["result"]["solution"],SECRET);
    }).await;
}
...
async fn served_quiz_with_arbitrary_task_id_stays_private_after_one_answer() {
    TestDb::with(|db|async move{
        ...
        scratch.tasks.insert(task.to_owned(),TaskProgress{task_id:task.to_owned(),task_type:"quiz".into(),
            total:2,served:1,answered:0,done:false,current_kp:None});
        put_state(&db,user,&scratch).await;
        let (status,queued)=create(&app,user,task,json!({"problem_id":PROBLEM_ID,"report_kind":"served"})).await;
        assert_eq!(status,StatusCode::OK,"{queued}");
...
async fn diagnostic_report_freezes_owned_answer_and_tracked_run_delta() {
    TestDb::with(|db|async move{
        ...
        let (status,raw)=call(&app,Method::POST,"/api/diag/start",Some(user),Some(json!({"course":"c1"}))).await;
        assert_eq!(status,StatusCode::OK,"{raw}");
        let problem=parse(&raw)["probe"]["problem_id"].clone();
        let (status,raw)=call(&app,Method::POST,"/api/diag/answer",Some(user),Some(json!({"problem_id":problem,"answer":"999"}))).await;
        assert_eq!(status,StatusCode::OK,"{raw}");
        let request=json!({"problem_id":problem,"report_kind":"diagnostic","answer":"FORGED"});
        assert_eq!(create(&app,other,"diag",request.clone()).await.0,StatusCode::NOT_FOUND);
        assert_eq!(create(&app,user,"wrong-task",request.clone()).await.0,StatusCode::NOT_FOUND);
        ...
        let run:Value=sqlx::query_scalar("SELECT to_jsonb(r) FROM problem_report_diagnostics r WHERE user_id=$1")
            .bind(user).fetch_one(&db.admin).await.unwrap();
        assert_eq!(frozen["task_policy"]["diagnostic_run_id"],run["run_id"]);
        assert_eq!(frozen["task_policy"]["diagnostic_curriculum_digest"],run["curriculum_digest"]);
        let delta=frozen["task_policy"]["diagnostic_delta"].as_object().unwrap();
        assert!(delta.values().any(|value|value.as_f64().unwrap()>0.0));
        assert_eq!(delta.len(),run["state"]["balances"].as_object().unwrap().len());

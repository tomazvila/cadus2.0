...
- answer_expr is a STRING in a restricted mathematical grammar, not Python. Use +, -, *, /, parentheses, and powers with a literal whole-number exponent (a**2, not a**b). Available evaluated functions with exact arity: abs(a), sqrt(a), gcd(a,b), lcm(a,b), floor(a), ceiling(a), min(a,b), max(a,b), factorial(a), binomial(a,b), signcase(selector,[negative,zero,positive]), equalitylabel(left,right), divisibilitylabel(number,divisor), linearclass((a,b),(c,d)), primeclass(number), quotientremainder(quotient,remainder), powerform(coefficient,[base,exponent]), logequation(base,[exponent,result]), expequation(base,[exponent,result]), atandeg(ratio), compounding(count), quarterextremum(curve,[lo,hi,direction]), quartervalue(family,quarter), trianglelaw(given), factorlist(number), firstmultiples(number,count), primefactors(number), and repeatedfactors(number,count). Use floor(a/b) for integer quotient and a-b*floor(a/b) for remainder. No //, %, str(), round(), ** with a variable exponent, Python conditionals, comparisons, indexing, strings, or units in answer_expr. Write an assignment only with a required_assignment contract. The answer must fit the requested numeric/expression kind; never concatenate quotient/remainder prose.
- When exemplars do not share a reviewed policy, answer_contract may state the deterministic policy for this pending template. A label uses one text choice parameter, equalitylabel(left,right), divisibilitylabel(number,divisor), linearclass((a,b),(c,d)), primeclass(number), trianglelaw(given), logequation(base,[exponent,result]), or expequation(base,[exponent,result]) as answer_expr; the equation writers prove the integer power identity and require the label vocabulary to enumerate the result. An exact contract may use compounding(count) for (1+1/n)^n or powerform(coefficient,[base,exponent]) to preserve an unevaluated power. A required_assignment contract preserves an exact assignment such as y=d and requires the learner to include the same target label. A required_single_power contract uses powerform(1,[base,exponent]) to emit one power of the computed numeric literal base. A required_normalized_scientific_notation contract writes an exact terminating numeric answer as a standard coefficient times 10 to an integer power. A required_simplest_radical contract writes an exact numeric answer as an integer or a reduced rational coefficient times one squarefree integer root; rationalize every denominator. A coordinate contract may use quarterextremum(curve,[lo,hi,direction]) for the leftmost parent sine/cosine extremum on whole quarter turns 0 through 4. An exact numeric contract may use quartervalue(family,quarter), where family is a text choice of sin or cos and quarter is a whole index from 0 through 3. An approximate contract may use atandeg(ratio) for a certified inverse-tangent angle in degrees. A quotient-and-remainder contract uses quotientremainder(quotient,remainder). An ordered exact-list contract uses factorlist(number), firstmultiples(number,count), primefactors(number), or repeatedfactors(number,count). A flat multipart answer uses multipart(part1, part2), with arguments in contract part order; each argument is math or a supported label expression. For a unit contract, keep answer_expr numeric; the server attaches the declared unit. For an inequality-union contract, use excludepoint(variable,bound), lowerbound(variable,bound), or upperbound(variable,bound), with a one-value text choice for variable. For a bidirectional interval/inequality conversion, use convertnotation(source) with a required_inequality_notation contract; the computed expected answer fixes the required output notation for that instance. The server validates it and never approves the row.

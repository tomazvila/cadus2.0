...
fn hints() -> &'static [HintRecipe] {
    ...
    HINTS.get_or_init(|| {
        ...
        for shard in [
            ...
            include_str!("instruction_hints_arithmetic_02.json"),
            include_str!("instruction_hints_arithmetic_03.json"),
            include_str!("instruction_hints_arithmetic_04.json"),
            include_str!("instruction_hints_fractions_01.json"),
            ...
            include_str!("instruction_hints_systems_01.json"),
            ...
            let replacement_keys: std::collections::BTreeSet<_> =
                replacements.iter().map(|row| row.key.as_str()).collect();
            catalog.retain(|row| !replacement_keys.contains(row.key.as_str()));

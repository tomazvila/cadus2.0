...
fn rows() -> Vec<Value> {
    ...
    serde_json::from_str(
        &fs::read_to_string(
            root.join("docs/content-foundations/unit05-residual/templates-second.json"),
        )
        .unwrap(),
    )
    .unwrap()
...
#[test]
fn all_108_instances_pass_the_real_worker_and_match_exhaustive_samples() {
    let root = Path::new(env!("CARGO_MANIFEST_DIR")).join("../..");

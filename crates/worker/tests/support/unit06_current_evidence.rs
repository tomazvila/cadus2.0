...
use cadus_core::{answer::{AnswerContract, Outcome}, curriculum, learner::problem_text_hash, template};
...
pub fn source_matches(receipt: &Value, expected: &Value) -> Result<(), &'static str> {
    if &receipt["sources"] == expected { Ok(()) } else { Err("stale Unit06 technical receipt source binding") }
}
...
pub fn wrong_answer(answer: &str, contract: &AnswerContract) -> String {
    let wrong = if contract == &AnswerContract::RequiredSinglePower {
        let cadus_core::answer::Ast::Pow(base, exponent) = cadus_core::answer::parse(answer).unwrap() else {
        ...
        assert!(matches!(cadus_core::answer::check_contract(answer, &changed, AnswerContract::Exact),
            Outcome::Decided(verdict) if !verdict.correct), "single-power control must change the mathematical value");
        changed
        ...
        let expected_groups: Vec<usize> = options.iter().enumerate()
            .filter(|(_, aliases)| aliases.iter().any(|alias|
                alias.trim().eq_ignore_ascii_case(answer.trim())))
            .map(|(index, _)| index).collect();
        assert_eq!(expected_groups.len(), 1, "expected label must name one declared synonym group");
        let expected_group = expected_groups[0];
        let (wrong_group, wrong_aliases) = options.iter().enumerate()
        ...
        assert!(!options[expected_group].iter().any(|alias|
            alias.trim().eq_ignore_ascii_case(candidate.trim())),
        ...
        fields.join("; ")
    } else { format!("({answer})+1") };
    contract.validate_expected(&wrong).expect("negative control must retain the response type");
    assert!(matches!(cadus_core::answer::check_contract(answer, &wrong, contract.clone()),
        Outcome::Decided(verdict) if !verdict.correct), "negative control must be decidably wrong");
    wrong
}
...
pub fn current_receipt(root: &Path) -> Value {
    ...
    let keys: Vec<String> = rows.iter().map(|r| r["kp_id"].as_str().unwrap().to_owned()).collect();
    assert_eq!(keys.iter().collect::<BTreeSet<_>>().len(), 78);
    let specs = cli::select(&curriculum, &keys).unwrap();
    let mut authored = BTreeSet::new();
    for spec in &specs { for item in &spec.exemplars { authored.insert(problem_text_hash(item.problem.trim())); } }
    for topic in curriculum.topics() {
        if specs.iter().any(|s| s.topic_id == topic.id.as_str()) {
            if let Some(item) = &topic.diagnostic_exemplar { authored.insert(problem_text_hash(item.problem.trim())); }
        }
    }
    ...
    for (row, spec) in rows.iter().zip(&specs) {
        ...
        if key == "estimating-square-roots/kp3" {
            assert_eq!(doc.samples.iter().map(|s| s.expected.text()).collect::<BTreeSet<_>>(),
                BTreeSet::from(["1".to_owned(), "2".to_owned()]), "comparison family must exercise both orderings");
        }
        ...
        for tuple in walk.tuples {
            ...
            assert!(siblings.insert(hash.clone()), "{key}: repeated sibling problem");
            let finite_case = spec.finite.as_ref().and_then(|policy|
                policy.domain.case_for(&instance.text, &instance.answer, instance.answer_contract.as_ref()));
            if authored.contains(&hash) {
                assert!(finite_case.is_some_and(|case| case.role == curriculum::FiniteCaseRole::TaughtRehearsal),
        ...
        evidence.push(json!({"kp_id":key,"kind":"template","status":"technical_pass",

...
use cadus_core::{
    ...
    curriculum::{FiniteCaseRole, load_curriculum},
...
fn reviewed_domain(key: &str) -> Value {
    ...
    domains
        .get(key)
        .unwrap_or_else(|| panic!("unreviewed key: {key}"))
        .clone()
}
...
fn inspect(
    ...
    assert_eq!(
        serde_json::to_value(&doc.params).unwrap(),
        expected["params"]
    );
    assert_eq!(
        serde_json::to_value(&doc.constraints).unwrap(),
    ...
    if key == "difference-of-squares/kp1" {
        ...
        assert_eq!(
            finite
                ...
                .cases
                .iter()
                .filter(|case| case.role == FiniteCaseRole::PracticeFresh)
                .count(),
            9
        );
        assert_eq!(
            finite
                ...
                .cases
                .iter()
                .filter(|case| case.role == FiniteCaseRole::TeachOnly)
                .count(),
            1
        );
        let worked =
            "Factor $x^2-36$. Report lower_factor and upper_factor in increasing constant order.";
        let taught = finite
            .domain
            .cases
            .iter()
        ...
        assert!(
            spec.finite.is_none(),
            "new finite objective needs an explicit regression review"
        );
    }
    ...
    for binding in walk.tuples {
        ...
        assert!(
            practice_hashes.insert(item.instance_hash.clone()),
            "duplicate practice instance"
        );
        let finite_case = spec.finite.as_ref().map(|finite| {
            ...
            assert!(
                ...
                "duplicate semantic practice case"
            );
            case
        });

...
fn drafts() -> Vec<Value> {
    let mut paths: Vec<_> =
        std::fs::read_dir(root().join("docs/content-foundations/fractions-decimals/templates"))
            .unwrap()
            .map(|p| p.unwrap().path())
            .collect();
    paths.sort();
...
fn all_pending_templates_exhaust_the_real_gate_and_avoid_authored_and_sibling_problems() {
    ...
    let output = root().join("target/unit01/regression");
    ...
    assert_eq!(report["passed"], 79, "{report}");
    let mut instances = 0;
...
fn wrong_samples_small_spaces_and_wrong_contracts_are_rejected() {
    let row = drafts()
        .into_iter()
        .find(|r| r["kp_id"] == "fraction-of-a-number/kp1")
        .unwrap();
    let spec = spec("fraction-of-a-number/kp1");

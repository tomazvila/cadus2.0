...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    .fetch_all(db.pool())
    .await?;
    let mut occupied = BTreeMap::<(String, String), i64>::new();
    ...
    for spec in &specs {
        ...
        review.push(json!({"kp_id":key,"objective":spec.kp_name,"source_answer_kind":spec.answer_kind,"constraints":spec.constraints,"source_exemplars":spec.exemplars.len(),"existing_instruction_template_counts":occupied.iter().filter(|((kp,_),_)|kp==&key).map(|((_,kind),count)|(kind.clone(),*count)).collect::<BTreeMap<_,_>>(),"new_drafts":added,"refusals":generated.refusals,"solution_proposals":generated.solutions,"assessment_proposals":generated.assessment,"diagnostic":diagnostic,"review_required":["pedagogical completeness","objective and operand bounds","template family variety","held-out family independence","human digest approval"]}));
    }
    db.pool().close().await;
